...
| 2 | Docker image + STEP 4.5 entrypoint | ✅ **Complete & committed** — all originally-deferred follow-ups landed via Plan Phase 2 (docker-git-hooks, rm-shim, smoke-test, apply_agent_profile + permissive/standard templates, all wired into entrypoint + Dockerfile.hermes). |
| 3 | Artifact capture + cleanup command | ✅ **Complete & committed** — `lcm_sandbox/core/artifact_capture.py` implements STEP 6.1–6.7 (filesystem archival under `~/.lcm-sandbox/artifacts/<sandbox-id>/`; S3/DB deferred to Phase 5). New `lcm-sandbox cleanup` CLI subcommand. 8 unit tests; suite 93 passed / 2 skipped. |
| 4 | Container launch (`docker_launcher.py`) + `launch`/`stop`/`status` CLI commands | ✅ **Complete & committed** — launcher + CLI now ship the full hardening set (`--cap-drop=ALL`, `--security-opt=no-new-privileges`, `--read-only` rootfs with `--tmpfs` for `/tmp`, `/var/tmp`, `/run`, `/home/aiagent/.cache`) plus the `--egress-allowlist` flag (advisory env var until Phase 5 wires up host-side enforcement). Integration tests gated on the Hermes image being built locally. |
